    ...
    <Card className="bg-zinc-900/80 border-zinc-800 overflow-hidden group">
      ...
      <CardContent className="p-4">
        <div className="flex justify-between items-start mb-2">
          ...
          <Badge className="bg-zinc-800 text-zinc-300 hover:bg-zinc-800">RC-{nft.id.toString().padStart(3, "0")}</Badge>
        </div>
        <div className="flex justify-between items-center">
          <div className="text-sm text-zinc-400">
            Minted: {nft.minted}/{nft.maxMint}
          </div>
          <div className="flex justify-between items-center">
            <div className="text-md font-bold text-amber-500">
              {formatEther(nft.price)} {symbol}
            </div>
          </div>
        </div>
      </CardContent>
      <CardFooter className="p-0 grid grid-cols-2">
        ...
        <Button
          onClick={onMintClick}
          className="rounded-none py-4 bg-zinc-800 hover:bg-zinc-700 text-white border-t border-zinc-700 group-hover:bg-gradient-to-r group-hover:from-amber-500 group-hover:to-amber-600 group-hover:text-black transition-all duration-300"
        >
          Mint NFT
        </Button>
      </CardFooter>
...
export function NftRoomLevel({
  nft,
  onMintClick,
  onDetailClick,
  showMintButton,
}: {
  nft: RoomLevelNftType;
  onMintClick: () => void;
  onDetailClick: () => void;
  ...
  return (
    <Card className="bg-zinc-900/80 border-zinc-800 overflow-hidden group">
      ...
      <CardContent className="p-4">
        <div className="flex justify-between items-start mb-2">
          <h3 className="text-md font-medium text-white">{getNftFullName(nft)}</h3>
          <Badge className="bg-zinc-800 text-zinc-300 hover:bg-zinc-800">RL-{nft.id.toString().padStart(3, "0")}</Badge>
        </div>
        <div className="flex justify-between items-center">
          <div className="text-sm text-zinc-400">
            Minted: {nft.minted}/{nft.maxMint}
